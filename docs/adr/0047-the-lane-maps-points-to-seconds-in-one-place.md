...
# The lane maps points to seconds in one place
...
| E | `loupeSeams` | seconds → fraction | `max(0, min(1, …))` |
...
that subtracted it. [ADR-0023](0023-the-detail-pane-is-ruler-lane-brief-dock.md) closed the question
...
- **`x(atTime:)` must not.** A Trim handle at the very end draws at exactly `width`; a Seam band's
...
[ADR-0031](0031-a-growing-recording-states-the-engines-figures-or-nothing.md)'s rule is *no duration
...
It renders only while `draggingHandle` is set, which is `@State` with no seam to set it from outside;
