...
# 0037. A warning's colour belongs to its mark, not to its sentence
...
[ADR-0009](0009-runway-clock-disk-guard.md)'s amber Runway trade, and it is deliberately
...
letter, and it would add two owned colours to a palette [ADR-0019](0019-content-colour-palette.md)
...
**A third site of this fault is left in place, and ticketed rather than fixed.** `presetRow()` renders an
unencodable preset's plain reason ([ADR-0015](0015-adopted-file-faithful-or-refused.md)) as a
...
2 ch / 48 kHz / Float32, so all four presets encode all of them and no preset row ever renders its reason —
...
applying this rule there is not a one-line split: it is a choice between giving the preset row a mark
...
[ADR-0032](0032-measure-treatments-on-target-surface.md)'s rule rather than this one:
`sourceFormatLine` measured 2.27 : 1 Dark and 1.86 : 1 Light and now measures 6.46 and 5.27 as
`Color.primary.opacity(0.7)`. Measured against the preset rows' own captions it reads *dimmer* than they
