...
# 0048. An Export's subject is the Recording, not the path it had at launch
...
[ADR-0006](0006-library-is-a-folder.md) and [ADR-0020](0020-rename-refuses.md)
...
affected: it is snapshotted at launch ([ADR-0012](0012-non-blocking-export.md))
and reads a file it already has open, so it ran to completion invisibly and wrote the right bytes to
the right destination. **Only the report lost its subject**, which is why the fix is the
...
The object is what the store relocates, so holding it is what makes the report follow a rename — and
...
[ADR-0021](0021-growing-recording-re-adopted.md) re-adopts a file whose length changed, and
...
is the arrangement [ADR-0046](0046-export-refuses-once.md) has just finished
...
- **A re-adoption, then a rename.** ADR-0021 re-adopts a file whose length changed and the editor
  rebinds to the fresh object; the report stays on the object that was dropped, and a rename after
...
away the preset row the user had picked for it (ADR-0015) and dropped the dock back to a sticky that cannot
...
**Three cases where there were none**, and the ticket's own is one of them: a parked report follows a
`relocate`; a fresh reading of the same path still finds its report, which is the rule the
...
the encode and not just the report. `cancel()` bumps `jobID`, and the panel's completion closure does
