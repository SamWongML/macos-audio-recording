...
- The encoders' real limits: **AAC-LC** takes 1–8 channels (mono through 5.1 encode natively) but caps at **48 kHz**; **HE-AAC** ("Compact", 64 kbps) takes even channel counts only; **ALAC** takes any rate and up to 8 channels losslessly. So preserving the source mostly *works* — the genuine blocker cases are a sample rate above 48 kHz (disables the three AAC preset rows) and odd channel counts above 2 on Compact. Master/ALAC always works, the escape hatch for exotic files.
...
- **Disabled presets and the sticky preference.** [#9](https://github.com/SamWongML/macos-audio-recording/issues/9)'s Quality Preset is an app-wide sticky preference. When the sticky preset can't encode the selected file, the unusable preset rows show disabled with a reason and Export is blocked until a working preset row is chosen — **no silent fallback** (which would surprise the user with a much larger ALAC file and break "parameters snapshot at launch"). Disabling a preset row for one file is a display over the sticky preference, not a change to it; navigating to a normal Recording restores the sticky choice untouched.
...
- **No provenance.** Origin is never tracked, marked, or derived — only *capability* differences surface (a disabled preset's reason, a fixed Trim). Consistent with [ADR-0006](0006-library-is-a-folder.md): the folder is the truth, there is no marker to write and nothing to re-derive.
