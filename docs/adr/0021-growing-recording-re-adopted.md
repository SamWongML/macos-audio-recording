...
# 0021. A Recording that grew is re-adopted, not followed

`LibraryStore.reconcile` keeps the **same `Recording` object** for a file that survives a refresh, and
[ADR-0006](0006-library-is-a-folder.md) is why: the Library is an ordinary folder that Finder may
...
depth and the Dropouts are all `let`s taken at that moment. For a finalized master that is exactly right,
because [ADR-0003](0003-immutable-caf-master.md) makes it immutable. For a file **still being
written** it is wrong, and the same-object rule then made it permanently wrong: a master is created at
the first sound ([ADR-0016](0016-recording-starts-at-first-sound.md)) and grows for the rest of
...
- **Extended attributes must stay outside the data length, and they are.** Trim, Gain, the Source and
  the Dropouts are all xattrs, and writing one leaves `st_size` untouched (measured, including a 2 KB
  Dropouts blob). If that were not so, every gesture-end would silently re-adopt the open Recording and
...
- **The length is read with a bare `stat`, not `URL.resourceValues`.** Resource values are cached on
  ...
  ([ADR-0044](0044-single-recording-read-point.md)).
...
- **It fixes a case capture does not know about.** A large file still being copied into the Library by
  hand is adopted part-written today ([ADR-0015](0015-adopted-file-faithful-or-refused.md)) and
...
- **Make `frameCount` mutable and re-read it.** Everything else read at open — `sampleRate`, the
  channel count, the bit depth, the Dropouts, the Trim clamped against the duration — would have to be
  ...
  **Made unexpressible by [ADR-0044](0044-single-recording-read-point.md).**
