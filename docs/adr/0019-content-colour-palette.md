...
[ADR-0033](0033-playhead-is-an-instrument.md): the playhead comes off this list. It
...
selected Quality Preset, not focus rings, not the sidebar selection, and not any text in Dark
Mode. The system accent marks *what the user has selected*; indigo marks *the audio and the Trim*.

**Dropout bands stay neutral, and this is now a rule rather than a comment.** `DropoutBand.swift` already
says "Neutral, not tinted — it is absence, not alarm." A Dropout is audio that never arrived, so the
absence of `Signal` **is** the meaning; tinting it would say the opposite.

**Typography is wholly SF, four preset rows, no light weights.** Recording name at Body/13 semibold;
...
play/pause and the Export button. Not the lane, not the inspector preset rows, not the sidebar. This
...
[ADR-0033](0033-playhead-is-an-instrument.md): the active Trim handle's
...
**The selected Quality Preset gains a leading checkmark, always.** It is currently marked by
