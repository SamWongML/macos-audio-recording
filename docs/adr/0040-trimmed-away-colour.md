...
# 0040. The Trimmed-away half is authored, not filtered

`TrimTimeline.waveform` drew the audio **twice**: `shape.grayscale(1)` under a masked colour copy.
So the lane has **six** fills, not the three [ADR-0033](0033-playhead-is-an-instrument.md)
...
[ADR-0019](0019-content-colour-palette.md) chose the filter over `Rectangle().fill(.background.opacity(0.62))`
...
That is [ADR-0032](0032-measure-treatments-on-target-surface.md) again, and the same
