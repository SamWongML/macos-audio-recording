...
# 0010. Silence never ends a Recording, and every gap becomes a Dropout
...
host-time gap, so a dropped buffer and a rebuilt tap both land as a **Dropout** and the master is
...
the mark most needs to distinguish. The Dropout list costs nothing extra, because the writer already
computes every pad.
...
never race the `processRestoreEnabled` recovery ADR-0007 gives first blocker; longer than
essentially all in-content dead air, so a podcast's pauses do not manufacture Dropouts. It is
...
fault. The doubling spacing keeps restore's first blocker on this path too and puts the end about
...
Thirty seconds is roughly 3x the worst legitimate Dropout, which is now computable — the soft path is
...
**Sleep, fast user switching and quit are detected on notification and produce no Dropout.**
...
instead of a load-bearing path. None of them dropouts: they end the Recording, so there is no audio
...
**A recovered Dropout tells the user nothing at the moment.** Recovery is the whole point of ADR-0007;
...
dead. The report for a Dropout belongs to the file, not to the moment.
...
without activating the app.** ADR-0009's blocker to open a window was aimed at a *predictable* stop
...
**A Recording is marked, the mark is the Dropout list, and it describes the master.** One extended
attribute alongside ADR-0006's Trim, Gain and Source: `com.samwongml.apptape.dropouts`, a JSON array of
...
dropout. An unreadable or absent value reads as a clean Recording, which is the safe direction and the
...
**Every Dropout is recorded; only Dropouts a listener would notice are surfaced.** The threshold is a
single Dropout of **250 ms or a total of 250 ms**, which sits cleanly between the two populations — a
dropped buffer is 10.67 ms and a rebuild Dropout is ≥ 1 s by construction. The total rule catches what
a per-Dropout rule misses: fifty scattered micro-gaps that add up to real damage. Surfacing everything
would fire the badge on something inaudible and teach the user to ignore it, which costs exactly the
case it exists for.

**Dropouts draw as a hatched band with a minimum width of about 3 pt**, at true width inside issue #7's
loupe. A minimum width is required because #7 fixed the timeline at always-fits-the-width, so a
20-minute master is ~1.2 s per pixel and a 1 s Dropout is sub-pixel. Hatching rather than a colour is
...
*warning*, which is what it means. Sub-threshold Dropouts live in a one-line summary in the editor
rather than littering the lane, and the Library row carries a **subtle trailing glyph on surfaced
Recordings only** — the Library is where a user arrives weeks later, when the moment's report is
long gone. A hand-adopted file (issue #30) has no Dropout attribute and is unmarked, correctly: the app
