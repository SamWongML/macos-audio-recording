...
**Top to bottom: a lane under a time ruler, the summary, air, and the transport docked to the window's
bottom edge.**
...
**So the freed space became the summary: the master's provenance and shape.** Source, when it was
...
is precisely the smear this ADR exists to fix. Extra window height becomes air below the summary.
...
**Everything above the bar has a height that does not depend on which Recording is shown.** The Dropout
line was its own conditionally-present row and the summary dropped `Captured` or `Master` when it had
...
brief is now always exactly five rows** — Source, Captured, Format, Master, Dropouts — with an em dash
where there is nothing to say and `None` where there are no Dropouts, and the Dropout line is one of those
...
**A selected sidebar row drops its waveform row waveform, and its coloured glyphs yield.** macOS fills
...
`.tertiary` timestamp went with it. Selected, the scissors and the Dropout glyph take `.primary` and
...
row waveform is dropped on the selected row for a different reason — over a saturated fill it stops
...
with a Dropout line and four brief rows clamps the window at 552. *(Still the detail pane's own minimum, but it stopped being the binding one: since [#114](https://github.com/SamWongML/macos-audio-recording/issues/114) the declared `minHeight` is **604**, so the floor is that declaration's, not this pane's — and the window it produces is **960 × 656**, 604 plus the 52 pt title bar ([ADR-0038](0038-dock-fill.md)).)*
