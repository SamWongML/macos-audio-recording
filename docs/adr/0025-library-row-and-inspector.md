...
# 0025. The Library row is one line; the inspector states the Export

Issue [#78](https://github.com/SamWongML/macos-audio-recording/issues/78) asked what the Library
sidebar and the Export inspector look like under [ADR-0019](0019-content-colour-palette.md), once the
detail pane had proven the design language. Three structurally different answers were built and
switched in the running app — two-line rows with a flush inspector, one-line rows with a preset row list,
and name-only browse rows with an Export ladder — and judged against the real Library rather than a
mockup. **The one-line/preset rows answer was chosen.**
...
The trailing lane crowded because it carried a timestamp, a scissors, a Dropout glyph and a duration
...
row, and the exact minute is in the summary two panes to the right, on the Recording the row selects.
...
**The row waveform survives selection**, superseding
[ADR-0023](0023-detail-pane-layout.md)'s *"a selected sidebar row drops its
waveform row waveform"*. Dropping it left the row the user is looking at as the one blank row in the
...
The trimmed duration has not gone quiet: it is the input to every size estimate, and each preset row
prints its own.

**Quality is four preset rows on screen, not a `Picker`.** All four presets and all four size estimates
are visible at once, so the choice is compared rather than revealed one at a time by a menu. This is
also the only shape in which ADR-0019's *"the selected Quality Preset gains a leading
checkmark, always"* means anything — inside a `Picker` the checkmark lives in a menu nobody sees.
A preset row the source's format cannot encode faithfully states its own plain reason where its codec
would be, so every unusable preset row says why, not just the effective one (ADR-0015).

**The source's rate and channel count are said once, beneath the preset rows.** They come from the
*source*, so they are identical on all four preset rows; printing them per preset row was four copies of one
fact, and it wrapped every preset row onto two lines in a 276 pt pane. Issue #9's "codec, bitrate, rate and
...
[ADR-0032](0032-measure-treatments-on-target-surface.md): `controlBackgroundColor` and
...
[ADR-0036](0036-dock-boundary.md) and corrected by
[ADR-0038](0038-dock-fill.md): the dock is pinned with
