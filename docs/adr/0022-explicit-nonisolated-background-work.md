...
# 0022. Background work is explicitly `nonisolated`
...
- **`Measuring…` is left as it is, with no progress bar.** The inspector's Loudness preview resolves in
  ...
  ([ADR-0013](0013-loudness-clamped-gain.md)) — but the preview does not pass it.
...
- **The inverse hazard exists and is *not* addressed here.** `CaptureEngine` is unannotated, and so
  ...
  **Closed by [ADR-0043](0043-capture-run-accepts-its-world.md).**
  ...
  the annotation and its reason. What made it tractable rather than open-ended was the dropout ADR-0043
