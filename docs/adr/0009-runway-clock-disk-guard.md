...
# 0009. The disk guard is a Runway clock, not a byte count
...
dead*. The report belongs to the moment, not to the file forever.
...
over a full-screen app, is hostile. The notification is the report, and **clicking it opens the
...
The blocker reuses the panel message surface ADR-0008 introduced for a denied grant; it names the
