...
# 0046. An Export refuses once, and both callers ask

Whether an Export may start was a pure, ordered decision written out in four places. `ExportInspector`
held a three-rule `blocker` computed property plus a fourth rule inline one function up;
...
Recording the dock offered an `Export…` button whose click produced a *failure report* — where
[ADR-0042](0042-dock-states-the-blocker.md) promises a sentence.
...
[ADR-0012](0012-non-blocking-export.md) forbids exporting and only the
...
[ADR-0034](0034-single-empty-state.md) took `AppTape can't decode this file` *out* of the
...
Recording, so neither a blocker nor a failed report has anywhere to appear. The case exists so the
...
**`.unencodable` carries the preset row's specific reason and prints the generic line.** ADR-0042's trade is
that the dock names the situation and leaves the specifics to the preset row, because
[ADR-0041](0041-reason-outside-the-control.md) already states
...
`.failed` report in place until it is dismissed, which is what the inspector's `Retry…` relies on —
...
*(Amended by [ADR-0048](0048-export-identifies-by-recording.md): #127 closed that
...
**`Reason` owns its SF Symbol, so the dock draws every blocker through one call.** The still-capturing
...
[ADR-0022](0022-explicit-nonisolated-background-work.md) makes an unannotated type in this target
...
`presentSavePanel`. Every blocker returns before any AppKit is touched, which is what each one asserts.

**Three of the four blockers can now be seen in the canvas.** `unencodable` needed a doctored Library
...
[ADR-0048](0048-export-identifies-by-recording.md) not even through that, so its
...
is the accepted record of what the user reads. The coordinator's report renders in the same dock slot
...
preset row, and throwing it away to simplify a sentence the dock does not print would have been a loss for
...
*(Done in [ADR-0048](0048-export-identifies-by-recording.md), which deleted the
