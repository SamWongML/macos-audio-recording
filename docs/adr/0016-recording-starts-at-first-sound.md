...
master wall-clock true and consistent with ADR-0007's mid-Recording dropouts. It was
rejected because the head pad is unbounded in a way a dropout never is: arming the
...
the tapped process is killed — and a full callback stop is still ADR-0007's padded dropout.
...
continuous through dropouts, which are padded into the master. No app-owned "waiting"
