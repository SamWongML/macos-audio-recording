...
notification centre. `ExportInspector`, `TrimTimeline` and `RecordingSummary` had no preview at all:
...
`CaptureBuilding`, `RunwayProbing`, `CaptureReporting` and `RecordingReading` are all *doing* interfaces:
...
dropout. They are capture's own face; the editor is a reader of it.
...
cancel on navigating away versus the re-selection that must leave a finished report standing.
...
with no tap; the summary reads "44.6 MB and growing" with no file; the dock's four Export phases render
...
shortfall of the dropout.** With any rows in the sidebar the preview host dies inside SwiftUI's own
...
needs a dropout this change does not have an argument for.
