...
# 0007. Mid-Recording faults pad a Dropout rather than stopping the Recording
...
switching is folded into the sleep rule. Everything else is a dropout to pad or an event to ignore.
...
account for as a rebuild dropout is not padded, it is the end of the Recording. Waking does not start
a new one: a Recording begins by user gesture, always.

**Restore gets first blocker over rebuild.** ADR-0001 records that a restored process reports
`kAudioProcessPropertyIsRunningOutput` a beat before its audio reaches the tap, so an eager rebuild
would tear down a tap the OS was about to fix, converting a free recovery into a dropout. The rebuild
...
if the Source relaunched, so re-resolution happens there, where the dropout is already being paid.

**Dropout length comes from host time, not sample counts.** A rebuilt tap's sample timeline restarts at
...
`kAudioHardwarePropertyDefaultOutputDevice` and rebuild on change, which this ADR's dropout machinery
