...
[ADR-0021](0021-growing-recording-re-adopted.md) settled that a `Recording` describes the
...
- the summary's **`Master`** row states what the file weighs right now, from a bare `stat`.
...
- the **sidebar row waveform** is withheld, because the envelope is a scan of a prefix drawn as
...
- the **Export ladder's estimates** read `—`, because the dock two rows below already refuses the
  export ([ADR-0012](0012-non-blocking-export.md)) and a confident
...
- A capturing Recording is legible from the editor without the panel: the row counts up, the
  transport says how far in, the summary says what it weighs.
...
- The `Master` row's `stat` is still performed, but not by the view: `CaptureRun` publishes
  `masterByteCount` on the same 4 Hz gate as `elapsed`, and the summary reads it
  ([ADR-0044](0044-single-recording-read-point.md)). The row used to do the syscall in its
...
- **[ADR-0027](0027-transport-readout-width.md) can be undone one control to the
