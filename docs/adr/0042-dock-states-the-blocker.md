...
# 0042. The dock states the blocker; the control does not wear it

[ADR-0041](0041-reason-outside-the-control.md) moved a *reason* out of a disabled control.
This moves the **blocker itself**. The Export dock's blocked state was a `.borderedProminent` button
...
([ADR-0032](0032-measure-treatments-on-target-surface.md)). The enabled button is
...
not two. [ADR-0025](0025-library-row-and-inspector.md)'s column was verified
...
[ADR-0046](0046-export-refuses-once.md) folded the still-capturing sentence
...
The second **deliberately duplicates** what ADR-0041 already states on the preset row. The dock names the
situation and leaves the specifics to the preset row, because the alternative — one sentence generic enough
...
**3.89 : 1 in Light**, the same figure #119 rejected for the preset rows, in the same slot on the same
...
[ADR-0025](0025-library-row-and-inspector.md)'s single height holds and
...
[ADR-0037](0037-warning-colour-on-the-mark.md)'s mark test is not invoked
