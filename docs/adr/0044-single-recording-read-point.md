...
# 0044. A Recording is read once, at a single boundary
...
body, and they never gated construction. Moving them would buy a symmetry and cost a dropout.
...
`CaptureEngine` the Dropouts. The reader *calls* them; it does not absorb them. The reader itself is
