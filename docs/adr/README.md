...
Read the work area below before changing it, then follow related records and amendments. Numbers
preserve decision order; groups help find the decisions relevant to a task. All records share the
single AppTape context defined in [CONTEXT.md](../../CONTEXT.md).

## Reading amendments

An accepted record can have a later amendment to part of its decision. Read both; the newer rule
wins for the amended part and the earlier record still explains the remaining trade-offs.

- [0033](0033-playhead-is-an-instrument.md) changes 0019's playhead colour and shadow rules.
- [0034](0034-single-empty-state.md) changes 0023 and 0024's empty-state treatment.
...
### Library, adoption and file identity
...
### Visual language and measurement
...
## Adding or revisiting a decision

Use the `domain-modeling` skill's concise format: a short title and one to three sentences stating
the context, decision and why. Create a record when the decision is hard to reverse, surprising
without context, and based on a real trade-off. Optional sections earn their
place through useful alternatives, evidence or consequences. Existing detailed records retain
that evidence; the concise format applies to new records.

Find the highest four-digit filename and increment it. Preserve existing numbers and filenames
when organizing the index. Add each new record once under its main work area. When a decision is
revisited, update its status or amendment pointer and make the current rule reachable from here.
Source files keep rationale out of comments; this index is the entry point.
