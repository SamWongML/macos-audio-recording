...
# 0041. A blocker's reason sits outside the control, not inside it

[ADR-0015](0015-adopted-file-faithful-or-refused.md) makes the Export inspector promise that
*every* unusable Quality Preset says why, not just the effective one. `presetRow()` kept that promise
...
96 kHz Recording that renders three unencodable preset rows at once — it came out at **1.58 : 1 in Dark and
1.22 : 1 in Light**, the worst figures this app has recorded, for the one sentence on the row that
has to be read. [ADR-0037](0037-warning-colour-on-the-mark.md) named this
site and declined to fix it blind, on
[ADR-0032](0032-measure-treatments-on-target-surface.md)'s rule. It was right to: the
...
| | before | after |
|---|---|---|
| unencodable preset row, reason, Dark | **1.58 : 1** | **11.71 : 1** |
| unencodable preset row, reason, Light | **1.22 : 1** | **13.02 : 1** |
...
2. **The explicit `.opacity(0.5)`** on the preset row.
...
What distinguishes a blocker from a codec label instead costs no width at all. **The name above it is
dimmed and the sentence is not** — an inversion that exists on no other row — and the preset row states no
size.

## An unencodable preset row states no size, which is ADR-0031's rule reaching a second cause

[ADR-0031](0031-growing-recording-figures.md) already refuses a
confident size for an export that cannot happen, for a Recording still arriving. A preset row that cannot
...
button, because the button already speaks it (ADR-0025 — the preset row speaks once).

## Considered options

**Darkening the orange to an owned stop was rejected**, for ADR-0037's reason: it adds a colour to a
palette [ADR-0019](0019-content-colour-palette.md) holds at two, and here it would not have worked
...
in the Library that renders it; a tick on a preset row that cannot encode needs `defaults write
...
[`prototype/unencodable-preset row`](https://github.com/SamWongML/macos-audio-recording/tree/prototype/unencodable-preset row/tools/unencodable-preset row).
