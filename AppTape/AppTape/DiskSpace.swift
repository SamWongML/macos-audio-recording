...
/// The Export pre-flight: before a byte is written, the estimate is compared against
...
/// clock**. Export is bounded work of known size; the argument that sent capture to a
...
enum DiskSpace {
    ...
    /// free space. makes this a **plain byte comparison**, not a Runway clock — the only
    ...
    /// slipping past the band still surfaces as a plain failure with the temp discarded.
    ...
    /// guard's read. Two things separate it from `freeBytes(forVolumeContaining:)`: it
