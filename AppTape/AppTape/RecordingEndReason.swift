...
nonisolated enum RecordingEndReason: Equatable, Sendable, CaseIterable {
    ...
    case userStopped
    /// The disk guard fired — the Runway ran out.
    case diskGuard
    /// Three rebuild attempts were spent and the tap still faults.
    case recoveryExhausted
    /// A rebuilt tap's format did not match the master's ASBD, so the timeline cannot continue
    ///. Not a retry — it ends at once without spending an attempt.
    case formatMismatch
    /// System sleep, or fast user switching folded into it. Caught on notification while
    /// still awake, so there is no gap to reconcile and the master ends at its last real sample.
    case sleep
    /// App quit or logout. Stops and saves unwarned, as already accepts for a left-click.
    case quit

    /// The four ends the user did not choose. Only these notify.
