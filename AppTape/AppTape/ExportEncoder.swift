...
/// write, the trimmed frame range, and the preset. A value, so later edits to Trim or preset never
/// reach a running encode.
...
///
/// This is the **Export/Loudness math seam** (issue #9/#24): every sample the export writes passes
/// through `transform(_:)`, which is the identity until Ticket 6 fills in Loudness measurement and
/// the correction constant. The seam is here, in the write loop, so that when normalization lands
/// it is a change to one function rather than a new pass.
///
...
final class ExportEncoder: @unchecked Sendable {
    ...
    func run(_ request: ExportRequest, onProgress: (Double) -> Void) throws {
        ...
        var client = Self.interleavedFloat(rate: rate, channels: channels)
        ...
        try scratch.withUnsafeMutableBufferPointer { buffer in
            ...
            while framesWritten < total {
                ...
                transform(buffer, sampleCount: Int(frames) * channels)
                ...
                onProgress(Double(framesWritten) / Double(total))
            }
        }
    }

    /// The Export/Loudness seam. Identity today: the export writes the trimmed master faithfully.
    /// Ticket 6 (issue #24) fills this in with the Loudness correction and hand-set Gain, applied
    /// after measurement — the one place a scaling rule lives, so ADR-0005's float→int24 clipping
    /// concern is decided here too. The `× 1.02` size estimate never moves with it (ADR-0012).
    private func transform(_ buffer: UnsafeMutableBufferPointer<Float>, sampleCount: Int) {
        // No-op until Ticket 6. Kept as a named seam so normalization is a change to this function.
        _ = sampleCount
    }
    ...
    /// Interleaved, packed Float32 — the client format both sides of the encode speak, at the
    /// source's own rate and channel count so nothing is resampled or downmixed.
    private static func interleavedFloat(rate: Double, channels: Int) -> AudioStreamBasicDescription {
