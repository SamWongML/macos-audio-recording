...
struct RowRecordGlyphTests {
    @Test func aRowThatIsNotRecordingIsIdle() {
        #expect(
            RowRecordGlyph.state(isRecordingTarget: false, sincePress: nil, hasFirstSound: false) == .idle)
        // Even if some other row is mid-bring-up, this one stays a plain circle.
        #expect(
            RowRecordGlyph.state(isRecordingTarget: false, sincePress: 2.0, hasFirstSound: false) == .idle)
    }

    @Test func aFreshPressHoldsTheCircleThroughTheGrace() {
        #expect(
            RowRecordGlyph.state(isRecordingTarget: true, sincePress: nil, hasFirstSound: false) == .armed)
        #expect(
            RowRecordGlyph.state(isRecordingTarget: true, sincePress: 0.2, hasFirstSound: false) == .armed)
        #expect(
            RowRecordGlyph.state(isRecordingTarget: true, sincePress: 0.49, hasFirstSound: false) == .armed)
    }

    @Test func bringUpShowsInFlightFromAboutFiveHundredMilliseconds() {
        #expect(
            RowRecordGlyph.state(isRecordingTarget: true, sincePress: 0.5, hasFirstSound: false) == .inFlight)
        #expect(
            RowRecordGlyph.state(isRecordingTarget: true, sincePress: 90, hasFirstSound: false) == .inFlight)
    }
    ...
    @Test func theFirstSoundIsRecordingRegardlessOfHowFastItArrived() {
        ...
        #expect(
            RowRecordGlyph.state(isRecordingTarget: true, sincePress: 0.1, hasFirstSound: true) == .recording)
        #expect(
            RowRecordGlyph.state(isRecordingTarget: true, sincePress: 5.0, hasFirstSound: true) == .recording)
    }
