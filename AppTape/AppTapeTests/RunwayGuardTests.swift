...
struct RunwayGuardTests {
    ...
    @Test func runwaySubtractsTheFloorBeforeDividingByRate() {
        ...
        let runway = RunwayGuard.runwaySeconds(
            freeBytes: Self.floor + 2_000_000_000, ratePerSecond: Self.rate)
    ...
    @Test func staysNominalAboveThreeHoursAndTurnsAmberAtIt() {
        ...
        #expect(
            g.receive(freeBytes: Self.freeBytes(runway: 3 * 3600 + 60), ratePerSecond: Self.rate).tier
                == .nominal)
        // At 3 h: amber.
        #expect(
            g.receive(freeBytes: Self.freeBytes(runway: 3 * 3600), ratePerSecond: Self.rate).tier == .amber)
    }
    ...
    @Test func amberHoldsThroughTheHysteresisBandAndRecoversAboveIt() {
        ...
        // Drop to amber.
        #expect(
            g.receive(freeBytes: Self.freeBytes(runway: 2 * 3600), ratePerSecond: Self.rate).tier == .amber)
        // Recover to just inside the 15-min band above 3 h — still amber, no flapping.
        #expect(
            g.receive(freeBytes: Self.freeBytes(runway: 3 * 3600 + 14 * 60), ratePerSecond: Self.rate).tier
                == .amber)
        // Past 3h15m: back to nominal, silently.
        #expect(
            g.receive(freeBytes: Self.freeBytes(runway: 3 * 3600 + 16 * 60), ratePerSecond: Self.rate).tier
                == .nominal)
    }
    ...
    @Test func theWarningFiresOnceOnCrossingIntoThirtyMinutes() {
        ...
        #expect(
            g.receive(freeBytes: Self.freeBytes(runway: 40 * 60), ratePerSecond: Self.rate).shouldWarn
                == false)
        // Crossing 30 min: the one warning.
        #expect(
            g.receive(freeBytes: Self.freeBytes(runway: 30 * 60), ratePerSecond: Self.rate).shouldWarn == true
        )
        // Still low: never a second warning.
        #expect(
            g.receive(freeBytes: Self.freeBytes(runway: 20 * 60), ratePerSecond: Self.rate).shouldWarn
                == false)
        #expect(
            g.receive(freeBytes: Self.freeBytes(runway: 10 * 60), ratePerSecond: Self.rate).shouldWarn
                == false)
    }
    ...
    @Test func theWarningReArmsOnlyAfterRecoveringPastFortyFiveMinutes() {
        ...
        #expect(
            g.receive(freeBytes: Self.freeBytes(runway: 44 * 60), ratePerSecond: Self.rate).shouldWarn
                == false)
        #expect(
            g.receive(freeBytes: Self.freeBytes(runway: 25 * 60), ratePerSecond: Self.rate).shouldWarn
                == false)
        // Recover past 45 min: re-armed. A fresh drop warns again.
        #expect(
            g.receive(freeBytes: Self.freeBytes(runway: 46 * 60), ratePerSecond: Self.rate).shouldWarn
                == false)
        #expect(
            g.receive(freeBytes: Self.freeBytes(runway: 29 * 60), ratePerSecond: Self.rate).shouldWarn == true
        )
    }
    ...
    @Test func startIsAmberBetweenTheFloorAndThreeHours() {
        #expect(
            RunwayGuard.startDecision(freeBytes: Self.freeBytes(runway: 60), ratePerSecond: Self.rate)
                == .allowAmber)
        #expect(
            RunwayGuard.startDecision(freeBytes: Self.freeBytes(runway: 3 * 3600), ratePerSecond: Self.rate)
                == .allowAmber)
    }

    @Test func startIsPlainAboveThreeHours() {
        #expect(
            RunwayGuard.startDecision(freeBytes: Self.freeBytes(runway: 5 * 3600), ratePerSecond: Self.rate)
                == .allow)
    }
