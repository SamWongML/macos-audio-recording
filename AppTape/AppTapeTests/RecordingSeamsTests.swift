...
/// How a Recording surfaces the Seams it read: the 250 ms single-or-total rule, what draws in the
...
/// surfacing rule over Seams a Recording already has, so it needs none.
@MainActor
struct RecordingSeamsTests {
    ...
    @Test func aRebuildSeamSurfacesAndDrawsInTheLane() {
        // A 1 s rebuild Seam (well over 250 ms) plus a tiny overrun that does not draw.
        let recording = Recording.stub(seconds: 5, seams: [
    ...
    @Test func onlySubThresholdSeamsAreNotSurfacedButAreSummarised() {
    ...
    @Test func aSingleSeamInflectsToTheSingular() throws {
        ...
        #expect(resolved(try #require(recording.seamSummary)).hasPrefix("1 Seam · "))
    }

    /// Padded silence is spoken at the scale the user can act on: milliseconds under a second,
    /// seconds above. A sub-threshold Seam is always under one, and a surfaced one usually over.
    @Test func paddingUnderASecondIsSpokenInMilliseconds() throws {
        ...
        let rendered = resolved(try #require(recording.seamSummary))
        #expect(rendered == "2 brief Seams · 200 ms padded, too short to hear")
    }

    @Test func paddingOverASecondIsSpokenInSeconds() throws {
        let recording = Recording.stub(seconds: 10, seams: [
            Seam(start: 48_000, frames: 96_000, cause: .rebuild),
        ])
        let rendered = resolved(try #require(recording.seamSummary))
        #expect(rendered == "1 Seam · 2.0 s of silence padded in")
    }
}
