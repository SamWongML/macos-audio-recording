...
struct DropoutReconcilerTests {
    ...
    /// A contiguous stream: each chunk's host time advances by exactly its own duration. No Dropouts.
    ...
    /// `overrun` Dropout (no rebuild was in flight).
    @Test func aDroppedBufferPadsAnOverrunDropout() {
        var r = DropoutReconciler(sampleRate: rate)
        let chunk = 512
        var host = 0.0
    ...
    @Test func aRebuildGapPadsARebuildDropout() {
        var r = DropoutReconciler(sampleRate: rate)
        ...
        #expect(r.dropouts.count == 1)
        #expect(r.dropouts[0].cause == .rebuild)
        #expect(r.dropouts[0].start == 48_000)
        #expect(r.dropouts[0].frames == 48_000)
    }

    /// An untrusted timestamp cannot be reconciled: no Dropout even when the host times imply a gap.
    ...
    /// Sub-frame host-time jitter must not manufacture a one-frame Dropout on every chunk.
    ...
    /// Two separate faults record two Dropouts, each starting at the right master frame.
    @Test func multipleDropoutsCarryTheirOwnStarts() {
        var r = DropoutReconciler(sampleRate: rate)
        var host = 0.0
        ...
        #expect(r.dropouts.count == 2)
        #expect(r.dropouts[0].cause == .overrun)
        #expect(r.dropouts[0].start == 48_000)
        #expect(r.dropouts[1].cause == .rebuild)
        // Second Dropout begins after: chunk1 + gap1(0.5s) + chunk2.
        #expect(r.dropouts[1].start == 48_000 + 24_000 + 48_000)
    }
}
