...
## Three findings before any layout question
...
Verified that the swap is clean, since a prototype that quietly shows two menu bar items
would misrepresent K. Under K, K's own item takes the exact slot the SwiftUI one had:

```
variant=I  ownStatusItem=none        NSStatusBarWindow(x=1273 w=32)  NSStatusBarWindow(x=0 w=32)
variant=K  ownStatusItem=x=1273 w=32 NSStatusBarWindow(x=1273 w=32)  NSStatusBarWindow(x=0 w=32) ...
```

`isInserted: false` genuinely removes it. Exactly one item is in the bar either way.
...
### 3. `withObservationTracking` keeps an `NSStatusButton` live, but only if it re-arms
...
(`menuBarLabel` is still counting under K, even with the item out of the bar: SwiftUI
keeps evaluating a scene it is not showing. Harmless at 1 Hz, but worth knowing.)
