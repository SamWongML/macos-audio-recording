...
enum Library {
    /// A dedicated folder for THIS prototype, so its five loudness fixtures don't mix with the
    /// editor-window prototype's 28 in ~/Music/AppTape. The shipping Library is ~/Music/AppTape
    /// (ADR-0006); the folder is a prototype convenience, nothing more.
    static var folder: URL {
        FileManager.default.urls(for: .musicDirectory, in: .userDomainMask)[0]
            .appending(path: "AppTape-LevelControls", directoryHint: .isDirectory)
    }
